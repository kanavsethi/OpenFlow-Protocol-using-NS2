...
//
  ...
  //
  onoff.SetAttribute ("Remote",
  AddressValue (InetSocketAddress (Ipv4Address ("10.1.1.9"), port)));
  app = onoff.Install (terminals.Get (9));
  app.Start (Seconds (1.0));
  app.Stop (Seconds (10.0));
  app = sink.Install (terminals.Get (0));
  app.Start (Seconds (0.0));
//
  ...
  //
  onoff.SetAttribute ("Remote",
  AddressValue (InetSocketAddress (Ipv4Address ("10.1.1.10"), port)));
  app = onoff.Install (terminals.Get (10));
  app.Start (Seconds (1.0));
  app.Stop (Seconds (10.0));
  app = sink.Install (terminals.Get (0));
  app.Start (Seconds (0.0));
//Gnuplot parameters
 ...
 std::string fileNameWithNoExtension = "FlowVSThroughput10to1";
...
AnimationInterface anim ("animation10to1.xml");
...
//Gnuplot ...continued
  ...
  monitor->SerializeToXmlFile("openflow10to1.flowmon", true, true);
  Simulator::Destroy ();
